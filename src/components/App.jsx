...
const App = () => {
  ...
  const renderPath = (paths) => {
    return (
      paths.map(({ id, d, stroke, fill, strokewidth }) => (
  ...
  const renderCircles = (circles) => {
    return (
      circles.map(({ id, cx, cy, r, stroke, fill, strokewidth }) => (
        <circle
          id={id}
          key={id}
          cx={cx}
    ...
    <div className="container container-fluid h-100 mt-5">
      <div className="row h-100">
        <aside className="col-6 col-md-4 col-lg-3 mt-2">
          ...
          {
            circles.length !== 0 &&
            (<span className="text">{renderLastPoint(circles)}</span>)
          }
        </aside>
        <main className="col">
          <div className="h-75">
